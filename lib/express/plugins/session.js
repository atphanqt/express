...
exports.Store.Memory = exports.Store.extend({
  ...
  fetch: function(sid, callback) {
    if(this.store[sid]) callback( null, this.store[sid] );
    else {
      this.newId(function(error, newId) { 
        puts("fetch->newId: " + newId)
          callback(null, new exports.Base(newId))
      })
    }
  },
  ...
  commit: function(session, callback) {
    ...
    callback( null , this.store[session.id] );
  },
  ...
  extend: {
    ...
    startReaper: function() {
      ...
      (function _reap() {
        ...
        }, self.reapInterval || self.reapEvery || (1).hour)
      })();
    }
  },
  ...
  on: {
    ...
    request: function(event, next) {
      ...
      if (!(sid= event.request.cookie('sid'))) {
         exports.Session.store.newId( function(error, newSid) { 
           if(error) next(error);
           else {
             sid= newSid;
             event.request.cookie('sid',sid, set('session cookie'))
             nextStep();
           }
         });
      }
